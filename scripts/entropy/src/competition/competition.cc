/*
  ...
  Competition manager for the game Spaghetti (CodeCup 2022).
  For more information: www.codecup.org.
  This program is part of the Caia project for the CodeCup 2022.
...
int main (int argc, char *argv[])
{
  ...
  { 
    ...
    for (ii=0; ii<aantal; ++ii)
    {
      ...
      LOCK(dummy);
      prflush("I number_players 2\n");
      prflush("I player 1 %s 30000 ../playerlogs/%s.blue.%s.%d.log\n", list[i].player, list[i].player, list[j].player,ii+1);
      prflush("I start 1\n");
      prflush("I player 2 %s 30000 ../playerlogs/%s.red.%s.%d.log\n", list[j].player, list[j].player, list[i].player, ii+1);
      prflush("I start 2\n");
      UNLOCK;
